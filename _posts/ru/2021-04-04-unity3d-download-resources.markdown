...
Поговорим о загрузке удалённых рессурсов в **Unity3d** приложение. В сети много материаллов на эту тему, но во многих из них информаця либо уже устарела, либо имеет существенные недостатки, речь о которых пойдёт в этой статье.
...
[GetSize](#getsize)                             | Применяется для того чтобы узанть размер файла во внешнем хранилище.
[GetText](#gettext)                             | Предназначен для скачивания файлов в текстовом формате.
[GetData](#getdata)                             | Позволяет получать из сети данные в виде масиива `byte`.
...
[GetVideoStream](#getvideostream)               | Предоставляет адрес видеофайла для его вспроиведения в виде потока.
[GetAssetBundle](#getassetbundle)               | Загружает заранее подготовленных файлов **AssetBundle**.
[GetAssetBundleVersion](#getassetbundleversion) | Оределияет актуальную версию **AssetBundle**. 
[GetHash128](#gethash128)                       | Cлужит для того чтобы извлечь hash из манифеста **AssetBundle**.
...
Он подробно рассмотрен в статье   | Сохраняет данные в памяти устройства и возвращает путь к уже закэшированным данным.

Прежде чем сохранять данные в памяти устройства, необходимо удостоверится в том, что данные на него поместятся. Для этого успешно применяется плагин **SimpleDiskUtils**


### SendWebRequest
Основным для класса `Network` является приватный асинхронный метод медод `WebRequest`, с помощью которого загружаются данные из сети. В качестве аргументов он принимает заранее подготовленный `UnityWebRequest`, `CancellationTokenSource`, `Action<float>`.
```csharp
...
```
Как можно заметить тело метода начинается с цикла ожидания говности кэша. Далее идёт отправка самого запроса: `request.SendWebRequest()`. После чего выполняется цикл ожидающий выполнения этого самого запроса. В течении этого цикла, через `Action<float>` в вызывающий метод передаётся прогресс выполнения этой операции. И в завершении метода запрос возвращается в вызывавший метод.

Для того чтобы прцесс ожидания кэша и выполнения запроса происходили асинхронно в циклы был добавлен вызов `await Task.Yield()`, к названию метода был добавлен модификатор async, а возвращаемый `UnityWebRequest` был обёрнут в `Task<>`. Про `await Task.Yield()` я узнал из статьи [Async/await в Unity](https://habr.com/ru/company/otus/blog/506962/). Ранее вместо него я пользовался вызовом `await new WaitForEndOfFrame()` из плагина  **AsyncAwaitUtil**, но решил отказаться в целях сокращения внешних зависимостей.

Крометого в обоих циклах проверяется флаг `cancelationToken.IsCancellationRequested`, через которого методу можно сообщить о необходимости прервать выполнение операций и вернуть `null`.

### GetSize
Метод используется для того чтобы узанть размер файла во внешнем хранилище. В качестве единственного аргумента принимает строку с url-адресом файла.
```csharp
...
### GetData
Метод `GetData` позволяет получать из сети данные в виде байтового масиива. На вход он принимает url-адрес файла в виде строки, токен для прерывания загрузки и `Action`, для отображения прогресса загрузки.
```csharp
...
```
В методе осуществляется отправка запроса через метод `SendWebRequest` описанный ранее. Метод так же является асинхронным и на выходе мы получаем данные в виде `Task` c массивом `byte`.

### GetTexture
Метод для плоучения изображения в виде `Texture2D`, принимает в качестве аргументов строку с url-адресом, `cancelationToken`, `Action` для отбражения прогресса и флаг опрделяющий, кэшировать загруженное изображение или нет. 
```csharp
...
```
В этом методе для скачивания изображения используется специализированный запрос `UnityWebRequestTexture.GetTexture(url)`. Кроме того перед отправкой запроса, проверяется наличие этого файла в памяти устройства. Для этого применятеся метод расширения `GetCachedPath(this string paht)`. Если файл с тем же именем и того же размера лежит по аналогичному локальному пути, url подменяется на путь к этому файлу. Метод возвращает объект типа `Texture2D`, обёрнутый в `Task`.

### GetAudioClip
Метод для плоучения аудиофайла в виде `AudioClip`, принимает в качестве аргументов строку с url-адресом, `cancelationToken`, `Action` для отбражения прогресса, флаг опрделяющий, кэшировать загруженный аудиофайл или нет, и `AudioType`, указывающий на формат аудиозаписи. Предпочтительный формат: **OGG**
```csharp
...
```
В этом методе для скачивания аудиофайла используется специализированный запрос `UnityWebRequestMultimedia.GetAudioClip(url, audioType)`. Кроме того перед отправкой запроса, проверяется наличие этого файла в памяти устройства. Для этого применятеся метод расширения `GetCachedPath(this string paht)`. Если файл с тем же именем и того же размера лежит по аналогичному локальному пути, url подменяется на путь к этому файлу.  Метод возвращает объект типа `AudioClip`, обёрнутый в `Task`.

### GetVideoStream
Метод предоставляет путь к видеофайлу на устройстве если он был ранее закэширован. Принимает в качестве аргументов строку с url-адресом, `cancelationToken`, `Action` для отбражения прогресса, и  флаг опрделяющий, кэшировать загруженный аудиофайл или нет.
```csharp
...
```
В методе сначала проверяется наличие этого файла в памяти устройства. Если файл с тем же именем и того же размера лежит по аналогичному локальному пути, возвращается путь к файлу на устройстве. Если такого файла нет - возвращается url-адрес файла и параллельно запускается процесс загрузки файла на устройство. Загрузка происходит с помощью метода `GetData` в заранее определённом делегате `AsyncOperation`, чтобы не ждать пока видео загрузится на устройство. Я пробовал запустить скачивание данных в одельном `Task`, с помощью `Task.Run()`, но это не сработало. В целом данный подход загрузки и воспроизведения видео не претендует на самый эффективный, но придумать лучше мне пока не удалось. Адрес файла возвращается в виде строки, он применяется для воспроизведения потоковых видеозаписей с помощью `VideoPlayer`'a.
 
> **Примечание:** Чтобы видео файл успешно воспроизводился, он должен соответсвовать формату потокового видео, а веб-сервер, к которому вы подключаетесь, должен соответствовать протоколу [**HLS**(HTTP Live Streaming)](https://ru.wikipedia.org/wiki/HLS)


### GetAssetBundle
Метод служит для загрузки заранее подготовленных файлов **AssetBundle**. Принимает в качестве аргументов строку с url-адресом, `cancelationToken`, `Action` для отбражения прогресса, и  флаг опрделяющий, кэшировать загруженный аудиофайл или нет.
```csharp
...
```
С начала в методе проверятеся наличие ранее закэшированной версии **AssetBundle** с помощью вспомогательного метода `GetAssetBundleVersion(Uri uri)` предсатвленного ниже. Если находится закэшированная весия совпадающая с той, что хранися по указанной ссылке, то загружается она. Если нет совпадений, то загружается версия AssetBundle из сети. Кэшируется новая версия или нет, зависит от состояния флага `caching`. Так же от этого флага звисит, будут ли удалены ранее загруженные версии. Метод возвращает `AssetBundle` обёрнутый в `Task`.
...
> **Внимание:** При кэшированнии файлов с одинаковым именем и локальным адресом возникнет коллизия и будет закэширован только последний загруженный файл. Такая ситуациия может возникнуть если в адресах загружаемых файлов отличаются только имена доменов.
...
```
При вызове метода url-адрес ковертирутся в путь к файлу и если он там находится, то его размер сравнивается с размером файла во внешнем хранилище. Если размеры совпадают то метод возвращает путь к файлу в виде строки, если нет - возвращает utl-адрес.
